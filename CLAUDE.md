...
- `NTSLiveMetadataService` (host-only, from `nts.live/api/v2/live`) is fetched only *after* playback reaches `.playing`, and merged titles avoid bumping `metadataUpdatedAt` when unchanged. Two non-obvious requirements: the API returns **HTML-escaped** titles (`GOD&#039;S WAITING ROOM`), decoded at the boundary by `decodingHTMLEntities()` so escaped text never reaches shared state; and the request sets `.reloadIgnoringLocalCacheData`, because the response carries `Cache-Control: max-age=900` and `URLSession.shared` was observed replaying a 40-minute-old title into the widget. The endpoint also sits behind CloudFront, so the origin value itself can still lag.
...
The status line's symbol is aligned with `VerticalAlignment.firstLineCenter` (in `Shared/FirstLineCenterAlignment.swift`), which puts it on the **first** line's cap-height midpoint. `.firstTextBaseline` leaves a small glyph sitting on the baseline; `.center` drifts downward as soon as the title wraps to two lines. `FirstLineCenterAlignmentTests` asserts the symbol position is independent of line count and renders PNGs (path printed as `ALIGNMENT_RENDER:`) so the layout can be eyeballed.
